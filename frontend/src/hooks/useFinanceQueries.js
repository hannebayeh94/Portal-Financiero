...
export function useExpensesByCategory(month, year) {
  return useQuery({
    queryKey: queryKeys.expensesByCategory(month, year),
    queryFn: () => get(`/expenses/by-category?month=${month}&year=${year}`),
  })
}
...
/**
 * Datos del Dashboard: un solo hook, caché compartida e invalidación granular.
 */
export function useDashboardData() {
  ...
  const results = useQueries({
    queries: [
      ...
      { queryKey: queryKeys.savingsSummary, queryFn: () => get('/savings/summary') },
      {
  ...
  const [monthly, income, expense, debts, savings, byCategory] = results
...
export function useSaveExpense(month, year) {
  const invalidate = useInvalidate()
  return useMutation({
    mutationFn: ({ id, payload }) =>
      id ? api.put(`/expenses/${id}`, payload) : api.post('/expenses', payload),
    onSuccess: () =>
      invalidate([['expenses'], ['reports', 'monthly-evolution']]),
  })
}
...
export function useAllCategories() {
  return useQuery({
    queryKey: queryKeys.categoriesAll,
...
export function useSaveCategory() {
  ...
  return useMutation({
    mutationFn: ({ id, payload }) =>
      id ? api.put(`/categories/${id}`, payload) : api.post('/categories', payload),
...
/* ============ Cuentas de ahorro activas (proyecciones) ============ */

export function useActiveSavings() {
  return useQuery({
    queryKey: ['savings', 'active'],
    queryFn: () => get('/savings?active=true'),
  })
}
...
export function useReportsData(year) {
  const results = useQueries({
    queries: [
      ...
      { queryKey: ['reports', 'monthly-evolution', year], queryFn: () => get(`/reports/monthly-evolution?year=${year}`) },
      ...
      { queryKey: ['reports', 'interest-projection'], queryFn: () => get('/reports/interest-projection?months=12') },
    ],
  })

  const [cashFlow, evolution, debts, savings, interests] = results

  return {
    loading: results.some((r) => r.isLoading),
    cashFlow: cashFlow.data,
    monthlyEvolution: evolution.data,
    debtStatus: debts.data,
    savingsStatus: savings.data,
    interestProjection: interests.data,
  }
}
